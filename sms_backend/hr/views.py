...
class PayrollItemViewSet(HrModuleAccessMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def pdf(self, request, pk=None):
        ...
        # Baseline payslip fallback
        content = (
            f"Payslip\nEmployee: {item.employee.employee_id}\nGross: {item.gross_salary}\n"
            f"Deductions: {item.total_deductions}\nNet: {item.net_salary}\n"
        )
        response = HttpResponse(content, content_type="text/plain")
        response["Content-Disposition"] = f'attachment; filename="payslip_{item.payroll.year}_{item.payroll.month:02d}_{item.employee.employee_id}.txt"'
        return response
