      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <div className="space-y-1">
            ...
            <select value={patientType} onChange={e => setPatientType(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
          ...
          {patientType === 'Student' ? (
            <div className="space-y-1">
              ...
              <select value={studentId} onChange={e => fillStudentName(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
          ...
          <div className="space-y-1">
            ...
            <input value={patientName} onChange={e => setPatientName(e.target.value)} placeholder="Full name" className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Referral Date *</label>
            <input type="date" value={referralDate} onChange={e => setReferralDate(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          </div>
          <input value={facilityName} onChange={e => setFacilityName(e.target.value)} placeholder="Facility / Hospital name *" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <input value={cost} onChange={e => setCost(e.target.value)} placeholder="Cost (Ksh)" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <input value={followUpDate} onChange={e => setFollowUpDate(e.target.value)} type="date" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Follow-Up Date (optional)</label>
          </div>
          <textarea value={reason} onChange={e => setReason(e.target.value)} placeholder="Reason for referral" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-2 lg:col-span-3" />
          <textarea value={diagnosis} onChange={e => setDiagnosis(e.target.value)} placeholder="Diagnosis" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-2 lg:col-span-3" />
          <textarea value={treatmentGiven} onChange={e => setTreatmentGiven(e.target.value)} placeholder="Treatment given at outside facility" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-2 lg:col-span-3" />
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Additional notes" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-2 lg:col-span-3" />
        </div>
