...
export default function AssetsAssignmentsPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await apiClient.post('/assets/assignments/', {
  ...
  const handleReturn = async (id: number) => {
    try {
      await apiClient.patch(`/assets/assignments/${id}/`, {
        status: 'Returned',
  ...
  const confirmDelete = async () => {
    ...
    setDeleteError(null)
    try {
      await apiClient.delete(`/assets/assignments/${deleteTarget.id}/`)
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <button
          ...
          className="mt-6 rounded-xl bg-emerald-500 px-6 py-2 text-sm font-semibold text-slate-900 transition hover:bg-emerald-400 disabled:opacity-50"
        >
            ...
            <tbody className="divide-y divide-slate-800/50">
              {assignments.map((row) => (
                <tr key={row.id} className="group hover:bg-slate-800/30">
                  ...
                  <td className="px-3 py-3">
                    <p className="text-slate-300">{row.assigned_date}</p>
                    {row.return_date && <p className="text-xs text-slate-500">Returned: {row.return_date}</p>}
                  </td>
                  <td className="px-3 py-3">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${
                      row.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-slate-700/40 text-slate-400'
                    }`}>
                      {row.status}
                    </span>
                  </td>
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete this assignment record for ${deleteTarget?.asset_name}?`}
        confirmLabel="Delete"
