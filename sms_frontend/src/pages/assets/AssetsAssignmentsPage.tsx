      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          <div className="space-y-1">
            ...
            <select required className="w-full rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm" value={assetId} onChange={(e) => setAssetId(e.target.value)}>
          ...
          <div className="space-y-1">
            ...
            <input required placeholder="Staff Name" className="w-full rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm" value={assignedToName} onChange={(e) => setAssignedToName(e.target.value)} />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Assigned By</label>
            <input required placeholder="Admin Name" className="w-full rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm" value={assignedByName} onChange={(e) => setAssignedByName(e.target.value)} />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Date</label>
            <input type="date" className="w-full rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm" value={assignedDate} onChange={(e) => setAssignedDate(e.target.value)} />
          </div>
        </div>
        <div className="mt-4 space-y-1">
          <label className="text-xs text-slate-400">Notes</label>
          <input className="w-full rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2 text-sm" value={notes} onChange={(e) => setNotes(e.target.value)} />
        </div>
          ...
          <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
            ...
            <div className="mt-4 space-y-3">
              <div className="space-y-1">
                ...
                <input value={returnedByName} onChange={e => setReturnedByName(e.target.value)} placeholder="Full name of person returning" className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
              </div>
              <div className="space-y-1">
                <label className="text-xs text-slate-400">Role / Department</label>
                <input value={returnedByRole} onChange={e => setReturnedByRole(e.target.value)} placeholder="e.g. Head of Department, Finance Clerk" className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
              </div>
            </div>
            {returnError ? <p className="mt-3 text-xs text-rose-300">{returnError}</p> : null}
            <div className="mt-5 flex gap-3 justify-end">
              <button onClick={() => setReturnTarget(null)} className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-200">Cancel</button>
