    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/70 p-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-2xl rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <button type="button" className="rounded-lg border border-slate-700 px-2 py-1 text-xs text-slate-200" onClick={onClose}>Close</button>
        </div>
        {children}
      </div>
    </div>
...
export default function AcademicsStructurePage() {
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      ...
      {isLoading ? <div className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-5 text-sm text-slate-300">Loading academic structure...</div> : null}
      ...
      <section className="col-span-12 grid gap-6 xl:grid-cols-2">
                ...
                {sortedYears.map((year) => (
                  <tr key={year.id} className="bg-slate-950/50">
                    ...
                    <td className="px-3 py-2"><div className="flex flex-wrap gap-2">
                      <button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => setCurrent(`/academics/years/${year.id}/`, 'Current academic year')}>Set current</button>
                      <button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => { setEditingYearId(year.id); setYearForm({ name: year.name, start_date: year.start_date, end_date: year.end_date, is_current: Boolean(year.is_current) }); setActiveModal('year') }}>Edit</button>
                      <button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => toggleActive(`/academics/years/${year.id}/`, year.is_active, 'Academic year')}>{year.is_active ? 'Archive' : 'Activate'}</button>
                      <button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => startCloneYear(year)}>Clone</button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-slate-800">
            <table className="w-full min-w-[760px] text-sm"><thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2 text-left">Year</th><th className="px-3 py-2 text-left">Name</th><th className="px-3 py-2 text-left">Billing</th><th className="px-3 py-2 text-left">Current</th><th className="px-3 py-2 text-left">Status</th><th className="px-3 py-2 text-left">Actions</th></tr></thead><tbody className="divide-y divide-slate-800">{terms.map((term) => <tr key={term.id} className="bg-slate-950/50"><td className="px-3 py-2">{term.academic_year_name ?? term.academic_year}</td><td className="px-3 py-2">{term.name}</td><td className="px-3 py-2">{term.billing_date || '--'}</td><td className="px-3 py-2">{term.is_current ? 'Yes' : 'No'}</td><td className="px-3 py-2">{term.is_active ? 'Active' : 'Archived'}</td><td className="px-3 py-2"><div className="flex flex-wrap gap-2"><button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => setCurrent(`/academics/terms/${term.id}/`, 'Current term')}>Set current</button><button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => { setEditingTermId(term.id); setTermForm({ academic_year: String(term.academic_year), name: term.name, start_date: term.start_date, end_date: term.end_date, billing_date: term.billing_date ?? '', is_current: Boolean(term.is_current), is_active: term.is_active }); setActiveModal('term') }}>Edit</button><button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => toggleActive(`/academics/terms/${term.id}/`, term.is_active, 'Term')}>{term.is_active ? 'Archive' : 'Activate'}</button></div></td></tr>)}</tbody></table>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="mt-4 overflow-x-auto rounded-xl border border-slate-800">
            <table className="w-full min-w-[760px] text-sm"><thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2 text-left">Name</th><th className="px-3 py-2 text-left">Order</th><th className="px-3 py-2 text-left">Description</th><th className="px-3 py-2 text-left">Status</th><th className="px-3 py-2 text-left">Actions</th></tr></thead><tbody className="divide-y divide-slate-800">{sortedGrades.map((grade) => <tr key={grade.id} className="bg-slate-950/50"><td className="px-3 py-2">{grade.name}</td><td className="px-3 py-2">{grade.order}</td><td className="px-3 py-2">{grade.description || '--'}</td><td className="px-3 py-2">{grade.is_active ? 'Active' : 'Archived'}</td><td className="px-3 py-2"><div className="flex flex-wrap gap-2"><button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => { setEditingGradeId(grade.id); setGradeForm({ name: grade.name, order: grade.order, description: grade.description || '', is_active: grade.is_active }); setActiveModal('grade') }}>Edit</button><button className="rounded-lg border border-slate-700 px-2 py-1 text-xs" onClick={() => toggleActive(`/academics/grade-levels/${grade.id}/`, grade.is_active, 'Grade level')}>{grade.is_active ? 'Archive' : 'Activate'}</button></div></td></tr>)}</tbody></table>
          </div>
        </div>
      ...
      {activeModal === 'year' ? (
        <ModalShell title={editingYearId ? 'Edit Academic Year' : 'New Academic Year'} onClose={closeModal}>
          <form className="grid gap-4" onSubmit={submitYear}>
            <label className="grid gap-1 text-xs text-slate-300">Name<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={yearForm.name} onChange={(e) => setYearForm((prev) => ({ ...prev, name: e.target.value }))} required /></label>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1 text-xs text-slate-300">Start date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={yearForm.start_date} onChange={(e) => setYearForm((prev) => ({ ...prev, start_date: e.target.value }))} required /></label>
              <label className="grid gap-1 text-xs text-slate-300">End date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={yearForm.end_date} onChange={(e) => setYearForm((prev) => ({ ...prev, end_date: e.target.value }))} required /></label>
            </div>
            ...
            <div className="flex justify-end gap-2"><button type="button" className="rounded-xl border border-slate-700 px-3 py-2 text-xs" onClick={closeModal}>Cancel</button><button type="submit" className="rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-slate-900">Save</button></div>
          </form>
        </ModalShell>
      ) : null}
      ...
      {activeModal === 'term' ? (
        <ModalShell title={editingTermId ? 'Edit Term' : 'New Term'} onClose={closeModal}>
          <form className="grid gap-4" onSubmit={submitTerm}>
            <label className="grid gap-1 text-xs text-slate-300">Academic year<select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={termForm.academic_year} onChange={(e) => setTermForm((prev) => ({ ...prev, academic_year: e.target.value }))} required><option value="">Select year</option>{sortedYears.map((year) => <option key={year.id} value={year.id}>{year.name}</option>)}</select></label>
            <label className="grid gap-1 text-xs text-slate-300">Name<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={termForm.name} onChange={(e) => setTermForm((prev) => ({ ...prev, name: e.target.value }))} required /></label>
            <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1 text-xs text-slate-300">Start date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={termForm.start_date} onChange={(e) => setTermForm((prev) => ({ ...prev, start_date: e.target.value }))} required /></label><label className="grid gap-1 text-xs text-slate-300">End date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={termForm.end_date} onChange={(e) => setTermForm((prev) => ({ ...prev, end_date: e.target.value }))} required /></label></div>
            <label className="grid gap-1 text-xs text-slate-300">Billing date (optional)<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={termForm.billing_date} onChange={(e) => setTermForm((prev) => ({ ...prev, billing_date: e.target.value }))} /></label>
      ...
      {activeModal === 'grade' ? (
        <ModalShell title={editingGradeId ? 'Edit Grade Level' : 'New Grade Level'} onClose={closeModal}>
          <form className="grid gap-4" onSubmit={submitGrade}>
            <label className="grid gap-1 text-xs text-slate-300">Name<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={gradeForm.name} onChange={(e) => setGradeForm((prev) => ({ ...prev, name: e.target.value }))} required /></label>
            <label className="grid gap-1 text-xs text-slate-300">Sort order<input type="number" min={1} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={gradeForm.order} onChange={(e) => setGradeForm((prev) => ({ ...prev, order: Number(e.target.value) }))} required /></label>
            <label className="grid gap-1 text-xs text-slate-300">Description<textarea className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={gradeForm.description} onChange={(e) => setGradeForm((prev) => ({ ...prev, description: e.target.value }))} rows={3} /></label>
      ...
      {activeModal === 'class' ? (
        <ModalShell title={editingClassId ? 'Edit Class Section' : 'New Class Section'} onClose={closeModal}>
          <form className="grid gap-4" onSubmit={submitClass}>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1 text-xs text-slate-300">Academic year<select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.academic_year} onChange={(e) => setClassForm((prev) => ({ ...prev, academic_year: e.target.value }))} required><option value="">Select year</option>{sortedYears.map((year) => <option key={year.id} value={year.id}>{year.name}</option>)}</select></label>
              <label className="grid gap-1 text-xs text-slate-300">Grade level<select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.grade_level} onChange={(e) => setClassForm((prev) => ({ ...prev, grade_level: e.target.value }))}><option value="">Select grade</option>{sortedGrades.map((grade) => <option key={grade.id} value={grade.id}>{grade.name}</option>)}</select></label>
            </div>
            <label className="grid gap-1 text-xs text-slate-300">Section name<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.section_name} onChange={(e) => setClassForm((prev) => ({ ...prev, section_name: e.target.value }))} required /></label>
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="grid gap-1 text-xs text-slate-300">Class teacher<select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.class_teacher} onChange={(e) => setClassForm((prev) => ({ ...prev, class_teacher: e.target.value }))}><option value="">Unassigned</option>{staff.map((member) => <option key={member.id} value={member.id}>{`${member.first_name} ${member.last_name}`.trim()}</option>)}</select></label>
              <label className="grid gap-1 text-xs text-slate-300">Room<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.room} onChange={(e) => setClassForm((prev) => ({ ...prev, room: e.target.value }))} /></label>
            </div>
            <label className="grid gap-1 text-xs text-slate-300">Capacity<input type="number" min={1} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={classForm.capacity} onChange={(e) => setClassForm((prev) => ({ ...prev, capacity: Number(e.target.value) }))} required /></label>
      ...
      {activeModal === 'clone' ? (
        <ModalShell title="Clone Academic Structure" onClose={closeModal}>
          <form className="grid gap-4" onSubmit={submitCloneYear}>
            <label className="grid gap-1 text-xs text-slate-300">New year name<input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={cloneForm.name} onChange={(e) => setCloneForm((prev) => ({ ...prev, name: e.target.value }))} required /></label>
            <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-1 text-xs text-slate-300">Start date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={cloneForm.start_date} onChange={(e) => setCloneForm((prev) => ({ ...prev, start_date: e.target.value }))} required /></label><label className="grid gap-1 text-xs text-slate-300">End date<input type="date" className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={cloneForm.end_date} onChange={(e) => setCloneForm((prev) => ({ ...prev, end_date: e.target.value }))} required /></label></div>
            ...
            <div className="flex justify-end gap-2"><button type="button" className="rounded-xl border border-slate-700 px-3 py-2 text-xs" onClick={closeModal}>Cancel</button><button type="submit" className="rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-slate-900">Clone</button></div>
          </form>
        </ModalShell>
      ) : null}
    </div>
