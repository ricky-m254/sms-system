    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {(['', 'open', 'resolved'] as const).map(f => (
          <button key={f} onClick={() => setFilterResolved(f)} className={`rounded-lg px-3 py-1.5 text-xs font-medium border transition ${filterResolved === f ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-200' : 'border-slate-700 text-slate-400 hover:bg-slate-800'}`}>
            ...
            {incidents.length === 0 ? (
              ...
              <tr key={i.id} className="hover:bg-slate-800/30 transition">
                ...
                <td className="px-4 py-3">
                  <div className="flex gap-2">
                    <button onClick={() => toggleResolve(i)} className="rounded-lg border border-slate-700 px-3 py-1 text-xs text-slate-200 hover:bg-slate-800 transition">{i.resolved ? 'Re-open' : 'Resolve'}</button>
                    <button onClick={() => openEdit(i)} className="rounded-lg border border-slate-700 px-3 py-1 text-xs text-slate-200 hover:bg-slate-800 transition">Edit</button>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl border border-slate-800 bg-slate-950 p-6 space-y-3">
            ...
            <select value={vehicleId} onChange={e => setVehicleId(e.target.value ? Number(e.target.value) : '')} className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm">
              <option value="">Select vehicle</option>
              {vehicles.map(v => <option key={v.id} value={v.id}>{v.registration}</option>)}
            </select>
            <input type="date" value={incidentDate} onChange={e => setIncidentDate(e.target.value)} className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm" />
            <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Incident description…" rows={3} className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm resize-none" />
            <div className="grid grid-cols-2 gap-3">
              <select value={severity} onChange={e => setSeverity(e.target.value)} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm">
                {SEVERITIES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
              <input value={reportedBy} onChange={e => setReportedBy(e.target.value)} placeholder="Reported by" className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm" />
            </div>
