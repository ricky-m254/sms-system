...
const navItems = [
  { label: 'Dashboard', to: '/modules/examinations' },
  { label: 'Exam Sessions', to: '/modules/examinations/sessions' },
  { label: 'Exam Papers', to: '/modules/examinations/papers' },
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans">
      <div className="mx-auto grid min-h-screen max-w-7xl grid-cols-12 gap-4 px-4 py-6 sm:gap-6 sm:px-6 sm:py-8">
        <aside className="col-span-12 rounded-2xl glass-panel p-5 md:col-span-3 lg:col-span-2">
          <ModuleToolbar currentModule="EXAMINATIONS" />
          ...
          <div className={`mt-4 space-y-2 text-sm ${open ? 'block' : 'hidden'} md:block`}>
            {navItems.map((item) => (
              <NavLink
                key={item.to}
                to={item.to}
                end={item.to === '/modules/examinations'}
