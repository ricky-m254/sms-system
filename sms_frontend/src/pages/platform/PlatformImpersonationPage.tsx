    ...
    <div className="grid grid-cols-12 gap-6">
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <form className="mt-4 grid gap-3 md:grid-cols-5" onSubmit={createSession}>
          ...
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Target username" value={form.target_username} onChange={(e) => setForm((p) => ({ ...p, target_username: e.target.value }))} required />
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Duration minutes" type="number" min={1} max={240} value={form.duration_minutes} onChange={(e) => setForm((p) => ({ ...p, duration_minutes: e.target.value }))} />
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={String(form.read_only)} onChange={(e) => setForm((p) => ({ ...p, read_only: e.target.value === 'true' }))}>
            <option value="true">Read-only</option>
            <option value="false">Read/Write</option>
          </select>
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Reason" value={form.reason} onChange={(e) => setForm((p) => ({ ...p, reason: e.target.value }))} />
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-[980px] w-full text-left text-sm">
            <thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2">Tenant</th><th className="px-3 py-2">Target</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Duration</th><th className="px-3 py-2">Requested By</th><th className="px-3 py-2">Actions</th></tr></thead>
              ...
              {sessions.map((row) => (
                <tr key={row.id} className={`bg-slate-950/50 ${selectedId === row.id ? 'ring-1 ring-emerald-500/50' : ''}`}>
                  ...
                  <td className="px-3 py-2 space-x-2">
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => setSelectedId(row.id)}>Select</button>
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => void actionSession(row.id, 'approve')}>Approve</button>
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => void startSession(row.id)}>Start</button>
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => void actionSession(row.id, 'end')}>End</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selected && authDump ? (
        <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <pre className="mt-3 overflow-x-auto rounded-xl border border-slate-800 bg-slate-950/60 p-4 text-xs text-slate-300">{authDump}</pre>
        </section>
      ) : null}
    </div>
  )
