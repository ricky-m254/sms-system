    ...
    <div className="grid grid-cols-12 gap-6">
      ...
      {slaOverview ? (
        <section className="col-span-12 grid gap-4 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Open tickets: {slaOverview.open_tickets}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Overdue first response: {slaOverview.overdue_first_response}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Overdue resolution: {slaOverview.overdue_resolution}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Avg resolution (hrs): {slaOverview.avg_resolution_hours}</div>
        </section>
      ) : null}
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <form className="mt-4 grid gap-3 md:grid-cols-4" onSubmit={createTicket}>
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={form.tenant} onChange={(e) => setForm((p) => ({ ...p, tenant: e.target.value }))} required>
            <option value="">Select tenant</option>
            {tenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={form.category} onChange={(e) => setForm((p) => ({ ...p, category: e.target.value }))}>
            {['TECHNICAL', 'BILLING', 'FEATURE', 'BUG'].map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={form.priority} onChange={(e) => setForm((p) => ({ ...p, priority: e.target.value }))}>
            {['LOW', 'NORMAL', 'HIGH', 'URGENT'].map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Creator email" value={form.created_by_email} onChange={(e) => setForm((p) => ({ ...p, created_by_email: e.target.value }))} />
          <input className="md:col-span-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Subject" value={form.subject} onChange={(e) => setForm((p) => ({ ...p, subject: e.target.value }))} required />
          <input className="md:col-span-2 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Description" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} required />
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <div className="flex flex-wrap gap-2">
          ...
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={tenantFilter} onChange={(e) => setTenantFilter(e.target.value)}>
            <option value="">All tenants</option>
            {tenants.map((item) => <option key={item.id} value={String(item.id)}>{item.name}</option>)}
          </select>
          <button className="rounded-lg border border-slate-700 px-3 py-2 text-sm" onClick={() => void loadData()}>Refresh</button>
        </div>
        <div className="mt-4 overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-[980px] w-full text-left text-sm">
            <thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2">Ticket</th><th className="px-3 py-2">Tenant</th><th className="px-3 py-2">Priority</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Subject</th><th className="px-3 py-2">Action</th></tr></thead>
            <tbody className="divide-y divide-slate-800">
              ...
              {filtered.map((row) => (
                <tr key={row.id} className={`bg-slate-950/50 ${selectedId === row.id ? 'ring-1 ring-emerald-500/50' : ''}`}>
                  ...
                  <td className="px-3 py-2"><button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => setSelectedId(row.id)}>Select</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {selected ? (
        <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-4">
            <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Assign user ID" value={assignUserId} onChange={(e) => setAssignUserId(e.target.value)} />
            <button className="rounded-lg border border-slate-700 px-3 py-2 text-sm" onClick={() => void assignTicket()}>Assign</button>
            <button className="rounded-lg border border-slate-700 px-3 py-2 text-sm" onClick={() => void actionTicket(selected.id, 'resolve')}>Resolve</button>
            <button className="rounded-lg border border-slate-700 px-3 py-2 text-sm" onClick={() => void actionTicket(selected.id, 'close')}>Close</button>
            <input className="md:col-span-3 rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Internal note" value={noteBody} onChange={(e) => setNoteBody(e.target.value)} />
            <button className="rounded-lg border border-slate-700 px-3 py-2 text-sm" onClick={() => void addNote()}>Add Note</button>
          </div>
        </section>
      ) : null}
    </div>
