    ...
    <div className="grid grid-cols-12 gap-6">
      ...
      {overview ? (
        <section className="col-span-12 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Open alerts: {overview.open_alerts}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Critical open alerts: {overview.critical_alerts}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Metric keys tracked: {overview.snapshot_metric_keys.length}</div>
        </section>
      ) : null}
      {alertSummary ? (
        <section className="col-span-12 grid gap-4 md:grid-cols-5">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Open: {alertSummary.open}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Acknowledged: {alertSummary.acknowledged}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Resolved: {alertSummary.resolved}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">Critical open: {alertSummary.critical_open}</div>
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4 text-sm">MTTR 30d (hrs): {alertSummary.mttr_hours_30d}</div>
        </section>
      ) : null}
      ...
      <section className="col-span-12 lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <form className="mt-4 grid gap-3" onSubmit={createSnapshot}>
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Metric key" value={snapshotForm.metric_key} onChange={(e) => setSnapshotForm((p) => ({ ...p, metric_key: e.target.value }))} required />
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={snapshotForm.tenant} onChange={(e) => setSnapshotForm((p) => ({ ...p, tenant: e.target.value }))}>
            <option value="">Platform-wide</option>
            {tenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Value (optional)" value={snapshotForm.value} onChange={(e) => setSnapshotForm((p) => ({ ...p, value: e.target.value }))} />
          <textarea className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" rows={3} placeholder='Payload JSON e.g. {"p95": 120}' value={snapshotForm.payload} onChange={(e) => setSnapshotForm((p) => ({ ...p, payload: e.target.value }))} />
      ...
      <section className="col-span-12 lg:col-span-6 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <form className="mt-4 grid gap-3" onSubmit={createAlert}>
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Alert title" value={alertForm.title} onChange={(e) => setAlertForm((p) => ({ ...p, title: e.target.value }))} required />
          <input className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" placeholder="Metric key" value={alertForm.metric_key} onChange={(e) => setAlertForm((p) => ({ ...p, metric_key: e.target.value }))} />
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={alertForm.tenant} onChange={(e) => setAlertForm((p) => ({ ...p, tenant: e.target.value }))}>
            <option value="">Platform-wide</option>
            {tenants.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <select className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" value={alertForm.severity} onChange={(e) => setAlertForm((p) => ({ ...p, severity: e.target.value }))}>
            {['INFO', 'WARNING', 'CRITICAL'].map((item) => <option key={item} value={item}>{item}</option>)}
          </select>
          <textarea className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm" rows={3} placeholder="Details" value={alertForm.details} onChange={(e) => setAlertForm((p) => ({ ...p, details: e.target.value }))} />
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-[920px] w-full text-left text-sm">
            <thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2">Title</th><th className="px-3 py-2">Metric</th><th className="px-3 py-2">Severity</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">Tenant</th><th className="px-3 py-2">Actions</th></tr></thead>
              ...
              {alerts.map((row) => (
                <tr key={row.id} className="bg-slate-950/50">
                  ...
                  <td className="px-3 py-2 space-x-2">
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => void alertAction(row.id, 'acknowledge')}>Acknowledge</button>
                    <button className="rounded border border-slate-700 px-2 py-1 text-xs" onClick={() => void alertAction(row.id, 'resolve')}>Resolve</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="col-span-12 rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="mt-4 overflow-x-auto rounded-xl border border-slate-800">
          <table className="min-w-[760px] w-full text-left text-sm">
            <thead className="bg-slate-900 text-xs uppercase tracking-wide text-slate-400"><tr><th className="px-3 py-2">Metric</th><th className="px-3 py-2">Value</th><th className="px-3 py-2">Tenant</th><th className="px-3 py-2">Captured</th></tr></thead>
            <tbody className="divide-y divide-slate-800">
