...
export default function StaffProfilesPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        await loadProfile(selectedStaffId)
      } catch {
        setError('Unable to load selected staff profile.')
      }
    }
    void load()
  }, [selectedStaffId])
  ...
  const addQualification = async () => {
    ...
    setNotice(null)
    try {
      await apiClient.post('/staff/qualifications/', {
        staff: selectedStaffId,
        qualification_type: 'Certificate',
        title: qualificationTitle.trim(),
      })
      setQualificationTitle('')
      await loadProfile(selectedStaffId)
      setNotice('Qualification added.')
    } catch {
      setError('Unable to add qualification.')
    }
  }
  ...
  const addEmergencyContact = async () => {
    ...
    setNotice(null)
    try {
      await apiClient.post('/staff/emergency-contacts/', {
        staff: selectedStaffId,
      ...
      setError('Unable to add emergency contact.')
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-900/60 p-4">
        ...
        >
          {staff.map((row) => (
            <option key={row.id} value={row.id}>{row.staff_id} - {row.full_name}</option>
          ))}
        </select>
      </section>
