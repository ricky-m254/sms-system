  ...
  return (
    <div className="space-y-6">
      <header className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        <h1 className="text-xl font-display font-semibold">Tournaments</h1>
        <p className="mt-1 text-sm text-slate-400">Schedule and track tournament results.</p>
      </header>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Tournament name *" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <select value={clubId} onChange={e => setClubId(e.target.value)} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
            <option value="">No club</option>
            {clubs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <input value={location} onChange={e => setLocation(e.target.value)} placeholder="Location" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <div className="space-y-1">
            <label className="text-xs text-slate-400">Start Date *</label>
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          </div>
          <div className="space-y-1">
            <label className="text-xs text-slate-400">End Date</label>
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          </div>
          <input value={positionAchieved} onChange={e => setPositionAchieved(e.target.value)} placeholder="Position achieved (e.g. 1st)" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <textarea value={result} onChange={e => setResult(e.target.value)} placeholder="Result summary" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-3" />
        </div>
        <div className="mt-4 flex gap-2">
          ...
          {editingId ? <button onClick={reset} className="rounded-xl border border-slate-700 px-4 py-2 text-sm text-slate-200">Cancel</button> : null}
        </div>
      </div>
