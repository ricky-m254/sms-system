    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Club name *" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <select value={clubType} onChange={e => setClubType(e.target.value)} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm">
            <option>Sports</option><option>Academic</option><option>Arts</option><option>Community</option><option>Other</option>
          </select>
          <input value={meetingDay} onChange={e => setMeetingDay(e.target.value)} placeholder="Meeting day (e.g. Wednesday)" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <input value={meetingTime} onChange={e => setMeetingTime(e.target.value)} placeholder="Meeting time (e.g. 4:00 PM)" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm" />
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Description" rows={2} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm sm:col-span-2" />
        </div>
